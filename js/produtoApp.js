angular.module('produtoApp', []).controller('ProdutoController', function ($scope, $http) {
  ...
  $scope.listarProdutos = function () {
    $http.get("https://prj-rubens-20221101.herokuapp.com/api/produto/")
  ...
  $scope.removerProduto = function (id) {
    let url = 'https://prj-rubens-20221101.herokuapp.com/api/produto/' + id;
    $http({
      method: "DELETE",
      url: url
    }).then(function (response) {
  ...
  $scope.salvarProduto = function () {
    ...
    if ($scope.produto.id > 0) {
      ...
      let url = 'https://prj-rubens-20221101.herokuapp.com/api/produto/';
      $http({
        method: tipo,
        url: url,
        data: $scope.produto
